...
link_listing = []
link_global = []
list_agent = [{'name':'Chrome','string':'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US) AppleWebKit/525.19 (KHTML, like Gecko) Chrome/1.0.154.53 Safari/525.19'},
...
def analyser():
    ...
    lecture = 0
    print bcolors.WARNING + "| starting forensics with jmp <" + current_jmp + ">" + bcolors.ENDC
    count_line = 0
    counter = 0
    breaked = 0
    old_line = 5
        ...
        if old_line >= len(source_code.split('\n')):
            ...
            for line in source_code.split('\n'):
                if counter <= old_line:
                    ...
                    count_line = 0
                    user_input = raw_input(bcolors.OKBLUE + "|" + hex(count_line) + "|" + bcolors.ENDC + '<enter> or <help>: ')
                    if "jmp:" in user_input:
                        ...
                        print """____________________________________________________________
| jmp:<0x??>     : (resume later analyser to this address)
| extract:<0x??> : (Extract tag information)
| mark:<0x??>    : (Use marker on this address)
| mov:<0x??>     : (Move to link)
